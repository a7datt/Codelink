...
async function startServer() {
    ...
    try {
      ...
      if (referralCode) {
        ...
        if (referrer) {
          ...
          // track referral count on users table
          const { data: refUser } = await supabase.from("users").select("referral_count").eq("id", referrer.id).single();
          await supabase.from("users").update({ referral_count: (refUser?.referral_count || 0) + 1 }).eq("id", referrer.id);
        }
      }
      ...
      const { data: user, error } = await supabase.from("users").insert({
        name, email, password_hash: hashedPassword, phone, referred_by_id: referredById
      }).select().single();
  ...
  app.post("/api/user/update", async (req, res) => {
    try {
      ...
      // Return full user object to avoid partial data issues
      const { data: updatedUser } = await supabase
        ...
        .single();
      if (updatedUser) {
        const stats = Array.isArray(updatedUser.user_stats) ? updatedUser.user_stats[0] : updatedUser.user_stats;
        delete updatedUser.user_stats;
  ...
  app.get("/api/leaderboard/:type", async (req, res) => {
    try {
      ...
      if (type === "topup") {
        ...
        // أكثر الأشخاص تفاعلاً (شحن + طلبات)
        const { data, error } = await supabase
          .from("user_stats")
          .select("user_id, total_recharge_sum, total_orders_count, profile_badge, users(name, avatar_url)")
        ...
        const result = (data || []).map((row: any) => ({
          ...
          avatar_url: row.users?.avatar_url || "",
          badge: row.profile_badge,
          value: `${row.total_orders_count || 0}`,
          unit: "طلب"
        }));
        return res.json(result);
  ...
  app.post("/api/admin/login", async (req, res) => {
    try {
      ...
      if (password === (setting?.value || "45654")) {
        res.json({ success: true });
      } else {
