...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Handle Admin Route
    if (window.location.pathname === "/adminCodelink2015") {
  ...
  const getTheme = () => {
    ...
    return {
      primary: "bg-[#1F76FF]",
      primaryHover: "hover:bg-[#1558CC]",
      text: "text-[#1F76FF]",
      textDark: "text-[#1558CC]",
      bgLight: "bg-blue-50",
      border: "border-blue-100",
      shadow: "shadow-blue-100",
      gradient: "from-[#1F76FF] to-[#1558CC]",
      icon: "text-[#1F76FF]",
      button: "bg-[#1F76FF]",
      buttonHover: "hover:bg-[#1558CC]"
    };
  };
    ...
    <header className="fixed top-0 left-0 right-0 h-16 bg-white border-b border-gray-100 flex items-center justify-between px-4 z-40">
      ...
      <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 flex items-center gap-2">
        <div className="w-10 h-10 flex items-center justify-center overflow-hidden">
          <img 
            src="https://i.ibb.co/LhCbbjnj/image.png" 
            alt="Logo" 
      ...
      return (
        <div className="px-4 space-y-6 pb-20 flex flex-col items-center justify-center min-h-[60vh]">
          ...
          <div className="w-full bg-white p-5 rounded-2xl border border-gray-100 shadow-sm space-y-3">
            ...
            <div className="flex justify-between text-sm"><span className="text-gray-500">المبلغ المدفوع</span><span className="font-bold text-[#1F76FF]">{finalPrice.toFixed(2)} $</span></div>
            {orderResult.replayApi?.length > 0 && (
              <div className="bg-gray-50 p-3 rounded-xl">
                ...
                {orderResult.replayApi.map((r: string, i: number) => (
                  <p key={i} className="text-sm font-mono font-bold text-[#1F76FF] break-all">{r}</p>
                ))}
              </div>
            )}
      ...
      <div className="fixed inset-0 z-[60] bg-gray-50 flex flex-col bottom-16">
            ...
            {messages.map((m) => (
              ...
              >
                <div className={`max-w-[85%] p-3 rounded-2xl shadow-sm relative ${
                  m.sender_role === 'user' ? 'bg-white text-gray-800 rounded-tr-none' : 'bg-[#1F76FF] text-white rounded-tl-none'
                }`}>
                  {m.type === 'bot_reply' && (
                  ...
                  {m.type === 'rating_request' ? (
                    <div className="text-center py-2">
                      ...
                      <button 
                        ...
                        className="w-full py-2 bg-white text-[#1F76FF] rounded-xl text-xs font-bold active:scale-95 transition-all disabled:opacity-50"
                      >
                        تأكيد التقييم
                      </button>
                    </div>
        ...
        <div className="p-4 bg-white border-t border-gray-100 flex items-center gap-2 shrink-0">
          ...
          <input 
            ...
            className="flex-1 bg-gray-50 border-none rounded-xl px-4 py-3 text-sm outline-none focus:ring-1 focus:ring-[#1F76FF] transition-all"
            value={newMessage}
      ...
      return (
        <div className="fixed inset-0 z-[60] bg-gray-50 flex flex-col bottom-16">
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4 no-scrollbar bg-[#F7F7F7]">
            <AnimatePresence initial={false}>
              {messages.map((m) => (
                ...
                >
                  <div className={`max-w-[85%] p-3 rounded-2xl shadow-sm relative ${
                    m.sender_role === 'admin' ? 'bg-[#1F76FF] text-white rounded-tr-none' : 'bg-white text-gray-800 rounded-tl-none'
                  }`}>
          ...
          <div className="p-4 bg-white border-t border-gray-100 flex items-center gap-2 sticky bottom-0">
            ...
            />
            <button onClick={() => handleSendReply(reply)} className="p-3 bg-[#1F76FF] text-white rounded-xl shadow-lg shadow-blue-100 active:scale-95 transition-all">
    ...
    return (
      <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 z-[60] bg-black/50 flex items-end" onClick={onClose}>
        <motion.div initial={{y:80}} animate={{y:0}} exit={{y:80}} className="bg-white w-full rounded-t-3xl p-6 space-y-4" onClick={e => e.stopPropagation()}>
          ...
          <div className="flex gap-2">
            <button onClick={() => setTarget("all")} className={`flex-1 py-2 rounded-xl text-xs font-bold ${target==="all"?"bg-[#1F76FF] text-white":"bg-gray-100 text-gray-600"}`}>للجميع</button>
            <button onClick={() => setTarget("one")} className={`flex-1 py-2 rounded-xl text-xs font-bold ${target==="one"?"bg-[#1F76FF] text-white":"bg-gray-100 text-gray-600"}`}>لمستخدم معين</button>
          </div>
          ...
          <button disabled={sending||!title||!body} onClick={async () => { setSending(true); await handleSendNotification(target==="one"&&uid?parseInt(uid):null, title, body); setSending(false); onClose(); }}
            className="w-full bg-[#1F76FF] text-white py-3.5 rounded-2xl font-bold text-sm disabled:opacity-40 flex items-center justify-center gap-2">
  ...
  const AdminAhminixTab = ({ categories, subcategories, subSubCategories, fetchCategories, fetchSubcategories, fetchSubSubCategories }: any) => {
    ...
    return (
      <div className="space-y-6 pb-8">
        <div className="bg-gradient-to-r from-[#1F76FF] to-[#1558CC] p-6 rounded-2xl text-white">
        ...
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="font-bold text-gray-800 flex items-center gap-2"><Wallet size={18} className="text-[#1F76FF]"/>حساب API الخارجي</h3>
            <button onClick={loadProfile} disabled={loadingProfile} className="bg-[#1F76FF] text-white px-4 py-2 rounded-xl text-xs font-bold disabled:opacity-50 flex items-center gap-1"><RefreshCw size={12} className={loadingProfile?"animate-spin":""}/>{loadingProfile?"جاري...":"تحديث"}</button>
          </div>
          {ahminixProfile ? (
        ...
        {/* تحديث الطلبات */}
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm space-y-4">
          <h3 className="font-bold text-gray-800 flex items-center gap-2"><RefreshCw size={18} className="text-[#1F76FF]"/>تحديث حالة الطلبات</h3>
          <p className="text-xs text-gray-500">يفحص الطلبات بحالة "processing" ويحدّثها تلقائياً</p>
          <button onClick={handleRefreshOrders} disabled={refreshLoading} className="w-full bg-[#1F76FF] text-white py-3 rounded-xl font-bold disabled:opacity-50 flex items-center justify-center gap-2"><RefreshCw size={16} className={refreshLoading?"animate-spin":""}/>{refreshLoading?"جاري التحديث...":"تحديث الطلبات"}</button>
        ...
        {/* فحص طلب */}
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm space-y-4">
          <h3 className="font-bold text-gray-800 flex items-center gap-2"><Search size={18} className="text-[#1F76FF]"/>فحص طلب</h3>
          <div className="flex gap-2">
            ...
            <button onClick={handleCheckOrder} disabled={checkLoading} className="bg-[#1F76FF] text-white px-4 py-3 rounded-xl font-bold text-sm disabled:opacity-50">{checkLoading?"...":"فحص"}</button>
          </div>
          {checkResult && (
        ...
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="font-bold text-gray-800 flex items-center gap-2"><Download size={18} className="text-[#1F76FF]"/>استيراد المنتجات</h3>
            <button onClick={loadProducts} disabled={loadingProducts} className="bg-[#1F76FF] text-white px-4 py-2 rounded-xl text-xs font-bold disabled:opacity-50 flex items-center gap-1"><RefreshCw size={12} className={loadingProducts?"animate-spin":""}/>{loadingProducts?"جاري الجلب...":"جلب المنتجات"}</button>
          </div>
          ...
          <div className="space-y-3">
            <div>
              ...
              <select value={targetSubcategoryId} onChange={e => { setTargetSubcategoryId(e.target.value); setTargetSubSubCategoryId(""); }} className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 text-sm outline-none focus:border-[#1F76FF]">
          ...
          {loadingProducts && (
            <div className="flex items-center justify-center py-8 gap-3">
              <RefreshCw size={20} className="animate-spin text-[#1F76FF]"/>
              ...
              <div className="space-y-2 max-h-72 overflow-y-auto rounded-xl border border-gray-100">
                {filteredProducts.length === 0 ? (
                  ...
                  <div key={p.id} onClick={() => toggleProduct(p.id)}
                    className={`flex items-center gap-3 p-3 cursor-pointer transition-all ${selectedProducts.includes(p.id)?'bg-blue-50 border-r-2 border-[#1F76FF]':'bg-white hover:bg-gray-50'}`}>
                    <div className={`w-5 h-5 rounded border-2 flex items-center justify-center flex-shrink-0 transition-all ${selectedProducts.includes(p.id)?'border-[#1F76FF] bg-[#1F76FF]':'border-gray-300'}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        ...
                        <span className="text-[10px] font-bold text-[#1F76FF]">{p.price} $</span>
          ...
          <button
            ...
            className="w-full bg-[#1F76FF] text-white py-3.5 rounded-xl font-bold disabled:opacity-40 flex items-center justify-center gap-2"
          >
  ...
  <div className="space-y-4">
    <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
      <div className="flex items-center justify-between mb-2">
        <div>
          <h3 className="font-bold text-gray-800 flex items-center gap-2"><Zap size={16} className="text-[#1F76FF]"/>وضع المعالجة</h3>
        ...
        {modeLoaded && (
          <div className="flex bg-gray-100 rounded-xl p-1 gap-1">
            ...
            <button onClick={() => toggleMode("manual")} disabled={modeLoading} className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${orderMode === 'manual' ? 'bg-[#1F76FF] text-white shadow-sm' : 'text-gray-500'}`}>يدوي</button>
          </div>
        )}
      </div>
    </div>
        ...
        {pendingAdminOrders.map(order => {
          ...
          return (
            <div key={order.id} className="border border-amber-100 rounded-xl bg-amber-50/40 p-4 space-y-3">
              <div>
                <p className="font-bold text-sm text-[#1F76FF]">#{order.id} - {order.product_name}</p>
        ...
        <div key={order.id} className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-xs font-bold text-[#1F76FF]">{(order.total_price || 0).toFixed(2)} $</p>
            </div>
          ...
          <div className="flex gap-2 overflow-x-auto pb-1">
            {[{val:"categories",label:"الأقسام"},{val:"subcategories",label:"الفرعية"},{val:"subSubCategories",label:"الفرع الفرعي"},{val:"products",label:"المنتجات"}].map(opt => (
              <button key={opt.val} onClick={() => { setElementsType(opt.val); loadElements(opt.val); }}
                className={`shrink-0 px-3 py-2 rounded-xl text-xs font-bold transition-all ${elementsType===opt.val?'bg-[#1F76FF] text-white':'bg-white border border-gray-100 text-gray-600'}`}>{opt.label}</button>
            ))}
          </div>
        )}
      ...
      {editingItem && (
        <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 z-[60] bg-black/60 flex items-center justify-center p-4" onClick={() => setEditingItem(null)}>
          <motion.div initial={{scale:0.92,opacity:0}} animate={{scale:1,opacity:1}} exit={{scale:0.92,opacity:0}} className="bg-white w-full max-w-md rounded-3xl p-6 space-y-4 max-h-[85vh] overflow-y-auto shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <button onClick={handleSaveEdit} className="w-full bg-[#1F76FF] text-white py-3.5 rounded-2xl font-bold text-sm mt-2">حفظ التعديلات</button>
          </motion.div>
        </motion.div>
      )}
    </AnimatePresence>
        ...
        {activeSubMenu === "fab_open" && (
          <motion.div initial={{opacity:0,y:30}} animate={{opacity:1,y:0}} exit={{opacity:0,y:30}}
            ...
            {[
              ...
              { key:"add_banner", label:"إضافة بانر", icon:<ImageIcon size={16}/>, color:"text-pink-600 bg-pink-50" },
              { key:"add_offer", label:"إضافة عرض", icon:<Tag size={16}/>, color:"text-rose-600 bg-rose-50" },
              ...
              <motion.div initial={{y:100,opacity:0}} animate={{y:0,opacity:1}} exit={{y:100,opacity:0}} className="bg-white w-full max-w-lg rounded-t-3xl p-6 space-y-4 max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
                ...
                {activeSubMenu === "add_category" && (
                  <div className="space-y-3">
                    ...
                    <button onClick={() => { handleAddCategory(); setActiveSubMenu(null); }} className="w-full bg-[#1F76FF] text-white py-3.5 rounded-2xl font-bold text-sm">إضافة القسم الرئيسي</button>
                  </div>
                )}
                {activeSubMenu === "add_subcategory" && (
                  <div className="space-y-3">
                    ...
                    <button onClick={() => { handleAddSubcategory(); setActiveSubMenu(null); }} className="w-full bg-[#1F76FF] text-white py-3.5 rounded-2xl font-bold text-sm">إضافة القسم الفرعي</button>
                  </div>
                )}
                {activeSubMenu === "add_subSubCategory" && (
                  <div className="space-y-3">
                    ...
                    <button onClick={() => { handleAddSubSubCategory(); setActiveSubMenu(null); }} className="w-full bg-[#1F76FF] text-white py-3.5 rounded-2xl font-bold text-sm">إضافة الفرع الفرعي</button>
                  </div>
                )}
                {activeSubMenu === "add_voucher" && (
                  <div className="space-y-3">
                    ...
                    <button onClick={() => { handleCreateVoucher(); setActiveSubMenu(null); }} className="w-full bg-[#1F76FF] text-white py-3.5 rounded-2xl font-bold text-sm">إنشاء القسيمة</button>
                  </div>
                )}
                {activeSubMenu === "add_paymentMethod" && (
                  <div className="space-y-3">
                    ...
                    <button onClick={() => { handleAddPaymentMethod(); setActiveSubMenu(null); }} className="w-full bg-[#1F76FF] text-white py-3.5 rounded-2xl font-bold text-sm">إضافة طريقة الدفع</button>
                  </div>
                )}
              </motion.div>
            </motion.div>
              ...
              <motion.div initial={{y:100,opacity:0}} animate={{y:0,opacity:1}} exit={{y:100,opacity:0}} className="bg-white w-full max-w-lg rounded-t-3xl p-6 space-y-4 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
                ...
                <div className="space-y-3">
                  ...
                  <button
                    ...
                    className="w-full bg-[#1F76FF] text-white py-3.5 rounded-2xl font-bold text-sm"
                  >
                    حفظ المنتج
                  </button>
                </div>
          ...
          {activeSubMenu === "add_balance" && (
            <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 z-[60] bg-black/50 flex items-end" onClick={() => setActiveSubMenu(null)}>
              <motion.div initial={{y:80}} animate={{y:0}} exit={{y:80}} className="bg-white w-full rounded-t-3xl p-6 space-y-4" onClick={e => e.stopPropagation()}>
                ...
                <button onClick={() => { handleManualTopup(); setActiveSubMenu(null); }} className="w-full bg-[#1F76FF] text-white py-3.5 rounded-2xl font-bold text-sm">شحن الرصيد</button>
              </motion.div>
            </motion.div>
          )}
        </AnimatePresence>
          ...
          {([
            ...
            if (item.tab === "fab") return (
              <button key="fab" onClick={() => setActiveSubMenu(activeSubMenu==="fab_open"?null:"fab_open")}
                className="relative -top-4 w-14 h-14 bg-[#1F76FF] text-white rounded-full shadow-lg shadow-blue-200 flex items-center justify-center active:scale-95 transition-all">
            ...
            return (
              <button key={item.tab} onClick={() => { setAdminTab(item.tab); setActiveSubMenu(null); }}
                className={`flex flex-col items-center gap-0.5 relative ${adminTab===item.tab?"text-[#1F76FF]":"text-gray-400"}`}>
                {item.icon}
                <span className="text-[9px] font-medium">{item.label}</span>
                {item.badge > 0 && <span className="absolute -top-1 -right-1 w-4 h-4 bg-[#1F76FF] text-white text-[8px] font-bold rounded-full flex items-center justify-center">{item.badge}</span>}
              </button>
            );
          })}
        </nav>
    ...
    <div className="min-h-screen bg-gray-50 text-right" dir="rtl">
            ...
            >
              <div className="p-8 text-center space-y-6">
                ...
                <div className="space-y-3 pt-2">
                  <button 
                    onClick={() => {
                      window.open(`https://t.me/codelink_sybot?start=${linkingModal.code}`, '_blank');
                    }}
      ...
      <AnimatePresence>
        {pageLoading && (
          ...
          >
            <div className="w-12 h-12 rounded-full border-4 border-gray-100 border-t-[#1F76FF] animate-spin" />
          </motion.div>
        )}
      </AnimatePresence>
    </div>
